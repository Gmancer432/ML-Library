...
def main(raw_training, raw_testing):
    ...
    ConvertColumn(raw_training, 14, float)
    ...
    trlabels = np.array([d[14] for d in raw_training])
    tstdata = expandData(raw_testing)
    
    # Train the Linear model
    classif = LMS(np.zeros(trdata.shape[1]))
    batchsize = 1000
    ...
    output = np.round(classif.PredictLabel(tstdata))
    SaveCSV([[int(i), output[int(i-1)]] for i in idxs], 'Outputs/Kaggle/LMS_GD_output.csv', ['ID', 'Prediction'])
